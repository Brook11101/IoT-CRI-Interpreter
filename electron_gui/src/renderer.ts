...
import {searchModuleName} from "./json_loader";
...
function refreshDSNSelect(elem: HTMLSelectElement, selectDict: object) {
    elem.innerHTML = "";
...
function getAllSelects() {
    ...
    return {
        ...
        trigger_DSN:
        trigger_DSN.options[trigger_DSN.selectedIndex].text,
...
window.onload = () => {
    ...
    //根据Trigger Device选项的变化来更新对应的Trigger Condition列表
    ipcRenderer.on("update-trigger-condition", (event, arg) => {
        ...
        refreshDSNSelect(trigger_DSN_elem, arg);
    });
    ...
    ipcRenderer.on("update-query-content", (event, arg) => {
        ...
        refreshDSNSelect(query_DSN_elem, arg)
    });
    ...
    ipcRenderer.on("update-action-execution", (event, arg) => {
        ...
        refreshDSNSelect(action_DSN_elem, arg)
    });
};
